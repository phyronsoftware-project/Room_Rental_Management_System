...
<main class="flex-1 overflow-y-auto bg-base">
    <div class="p-4 md:p-6 space-y-4">
        ...
        <div class="flex justify-between items-center">
            <h1 class="text-2xl font-bold text-primary-dark">Profile</h1>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div class="lg:col-span-1">
                <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-4 flex flex-col items-center">
                    <h2 class="text-base font-semibold text-slate-700 mb-3">Profile</h2>
                    ...
                    <form method="POST" action="{{ route('profile.avatar') }}" enctype="multipart/form-data"
                        ...
                        <div class="relative">
                            <img id="avatar-preview"
                                 src="{{ $avatar }}"
                                 class="w-28 h-28 md:w-32 md:h-32 rounded-full object-cover ring-2 ring-white shadow"
                                 alt="avatar">

                            {{-- camera badge --}}
                            <button type="button"
                                onclick="document.getElementById('avatar_file').click()"
                                class="absolute -right-1 -bottom-1 w-9 h-9 rounded-full bg-slate-200 hover:bg-slate-300 flex items-center justify-center shadow border border-white">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-slate-700" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor">
                        ...
                        <div class="mt-3 text-center">
                            <div class="text-base font-semibold text-slate-800">{{ $name }}</div>
                        ...
                        <button type="submit"
                                class="mt-3 w-full rounded-lg bg-blue-700 text-white py-2 text-sm font-semibold hover:bg-blue-800 transition">
                            Save Photo
                        </button>
                        ...
                        @if (session('avatar_ok'))
                            <div class="mt-2 text-xs text-emerald-700">{{ session('avatar_ok') }}</div>
                        @endif
                    </form>
                </div>
            </div>
            ...
            <div class="lg:col-span-2 space-y-4">
                ...
                {{-- Edit Details (ONLY: full_name, email) --}}
                <div class="bg-white rounded-xl border border-slate-200 shadow-sm">
                    <div class="px-4 py-3 border-b border-slate-200">
                        <h2 class="text-base font-semibold text-slate-800">Edit Details</h2>
                    </div>

                    <form method="POST" action="{{ route('profile.update') }}" class="p-4 space-y-4">
                        @csrf
                        ...
                        <div class="flex justify-end">
                            <button type="submit"
                                    class="rounded-lg bg-blue-700 text-white px-4 py-2 text-sm font-semibold hover:bg-blue-800 transition">
                                Save Changes
                            </button>
                        </div>

                        @if (session('profile_ok'))
                            <div class="text-xs text-emerald-700">{{ session('profile_ok') }}</div>
                        @endif
                    </form>
                </div>

                {{-- Change Password (ONLY password) --}}
                <div class="bg-white rounded-xl border border-slate-200 shadow-sm">
                    <div class="px-4 py-3 border-b border-slate-200">
                        <h2 class="text-base font-semibold text-slate-800">Change Password</h2>
                    </div>

                    <form method="POST" action="{{ route('profile.password') }}" class="p-4 space-y-4">
                        @csrf
                        ...
                        @if (session('pass_ok'))
                            <div class="text-xs text-emerald-700">{{ session('pass_ok') }}</div>
                        @endif
                    </form>
                </div>
